...
if(isset($_POST['order_btn'])){
    ...
    $cart_query = mysqli_query($conn, "SELECT * from `user_product` LEFT JOIN products ON `user_product`.`product_id` =`products`.`id` where `user_id`=$user_id") or die('query failed');
...
<section class="display-order">
    ...
    if(mysqli_num_rows($select_cart) > 0){
        while($fetch_cart = mysqli_fetch_assoc($select_cart)){
            ...
            ?>
            <p> <?php echo $fetch_cart['name']; ?> <span><?php echo '$'.$fetch_cart['price'].' / Quantity : '. $fetch_cart['quantity']; ?></span> </p>
            <?php
        }
    }else{
...
<section class="checkout">
    ...
    <form action="" method="post">
        ...
        <div class="flex">
            ...
            <div class="inputBox">
                <span>city :</span>
                <input type="text" name="city" required placeholder="Ex. Nablus">
            </div>
            <div class="inputBox">
                <span>country :</span>
                <input type="text" name="country" required placeholder="Ex. Palestine">
            </div>
